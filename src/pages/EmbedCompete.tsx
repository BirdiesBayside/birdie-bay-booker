...
import { Trophy, Medal, Award, Flame, Calendar, Users, Target, Zap, ChevronRight, Loader2 } from "lucide-react";
...
export default function EmbedCompete() {
  ...
  const { data: pastWinners = [] } = useQuery({
    queryKey: ["compete-past-winners", activeTour?.tour_id],
    enabled: !!activeTour?.tour_id,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("sgt_monthly_standings")
        .select("month, player_name, net_position, monthly_net_points")
        .eq("tour_id", activeTour!.tour_id)
        .eq("net_position", 1)
  ...
  const { data: stats } = useQuery({
    queryKey: ["compete-stats", activeTour?.tour_id],
    enabled: !!activeTour?.tour_id,
    queryFn: async () => {
      const [{ count: memberCount }, { count: roundsThisWeek }] = await Promise.all([
        supabase.from("sgt_tour_members").select("*", { count: "exact", head: true }).eq("tour_id", activeTour!.tour_id),
        currentTournament?.tournament_id
          ? supabase.from("sgt_scorecards").select("*", { count: "exact", head: true }).eq("tournament_id", currentTournament.tournament_id)
          : Promise.resolve({ count: 0 }),
      ]);
      return { memberCount: memberCount ?? 0, roundsThisWeek: roundsThisWeek ?? 0 };
    },
  });
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: CREAM, fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <section className="relative overflow-hidden" style={{ backgroundColor: GREEN }}>
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
          ...
          <div className="grid grid-cols-3 gap-3 sm:gap-6 max-w-2xl">
            <StatPill label="Tour Members" value={stats?.memberCount ?? "—"} icon={<Users className="h-4 w-4" />} />
            <StatPill label="Rounds This Week" value={stats?.roundsThisWeek ?? "—"} icon={<Zap className="h-4 w-4" />} />
            <StatPill label="Prize / Week" value="$40+" icon={<Trophy className="h-4 w-4" />} />
          </div>
        </div>
      </section>
      ...
      {/* THIS WEEK + MONTHLY WINNER side-by-side */}
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-6 grid lg:grid-cols-5 gap-6">
        {/* Weekly */}
        <Panel className="lg:col-span-3" tag="THIS WEEK" tagColor={ORANGE}
          title={currentTournament?.name ?? previousTournament?.name ?? "Weekly Tournament"}
          subtitle={currentTournament?.course_name ?? previousTournament?.course_name ?? "Birdies Tour"}>
        ...
        {/* Monthly */}
        <Panel className="lg:col-span-2" tag="MONTHLY WINNER" tagColor={GREEN}
          title={currentMonth} subtitle="Race for the title">
          {monthlyLoading ? (
            <LoadingRow />
        ...
        >
          ...
          <div className="relative grid lg:grid-cols-5 gap-8 items-start">
            <div className="lg:col-span-2">
              ...
              <h2 className="text-white font-black text-4xl sm:text-5xl leading-none mb-3"
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
          <div className="flex items-end justify-between mb-5">
            <div>
              ...
              <h3 className="font-black text-2xl sm:text-3xl" style={{ color: GREEN, fontFamily: "'Anton', sans-serif" }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {pastWinners.map((w: any) => (
              <div key={w.month} className="bg-white rounded-xl border p-4 shadow-sm" style={{ borderColor: BORDER }}>
                <div className="flex items-center gap-2 mb-2">
                  <Trophy className="h-4 w-4 text-yellow-500" />
                  <span className="text-[11px] uppercase tracking-widest font-bold" style={{ color: MUTED }}>
                    {w.month}
                  </span>
                </div>
                <p className="font-bold text-base truncate" style={{ color: GREEN }}>{w.player_name}</p>
                <p className="text-xs" style={{ color: MUTED }}>{w.monthly_net_points ?? 0} pts</p>
              </div>
            ))}
          </div>
        </section>
...
function StatPill({ label, value, icon }: { label: string; value: string | number; icon: React.ReactNode }) {
  return (
    <div className="bg-white/10 border border-white/15 rounded-2xl px-3 py-3 sm:px-4 sm:py-4 backdrop-blur-sm">
      <div className="flex items-center gap-1.5 text-white/60 text-[10px] sm:text-[11px] uppercase tracking-widest font-bold mb-1">
        {icon}{label}
      </div>
      <div className="text-white font-black text-2xl sm:text-3xl leading-none" style={{ fontFamily: "'Anton', sans-serif" }}>
        {value}
      </div>
    </div>
  );
...
function Panel({
  children, className, tag, tagColor, title, subtitle,
}: { children: React.ReactNode; className?: string; tag: string; tagColor: string; title: string; subtitle?: string }) {
  return (
    <div className={cn("bg-white rounded-2xl border shadow-sm overflow-hidden flex flex-col", className)} style={{ borderColor: BORDER }}>
      <div className="px-5 py-4 border-b" style={{ borderColor: BORDER }}>
        <span className="inline-block text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded text-white mb-2"
          style={{ backgroundColor: tagColor }}>
          {tag}
        </span>
        <h3 className="font-black text-xl truncate" style={{ color: GREEN, fontFamily: "'Anton', sans-serif" }}>
          {title}
        </h3>
        {subtitle && <p className="text-xs" style={{ color: MUTED }}>{subtitle}</p>}
      </div>
      <div className="px-5 py-2 flex-1">{children}</div>
    </div>
  );
}
